// src/services/DockerExecutionEngine.ts
...
class DockerExecutionEngine {
  ...
  private async loadExistingContainers(): Promise<void> {
    try {
      const containers = await this.docker.listContainers({ all: true }); // List all running containers
      for (const containerInfo of containers) {
        const container = this.docker.getContainer(containerInfo.Id);
        this.containers.set(containerInfo.Id, {
          id: containerInfo.Id,
  ...
  async startContainer(functionData: FunctionData): Promise<string> {
    ...
    try {
      ...
      console.log(`Container ${container.id} started for route ${route}`);
      return container.id;
...
if __name__ == "__main__":
  ...
  async executeFunction(functionData: FunctionData, input: any = {}): Promise<ExecutionResult> {
    ...
    try {
      ...
      console.log(`Executing function in container ${containerId}`);
      ...
      if (functionData.language === 'javascript') {
        ...
        execResult = await container.exec({
          Cmd: ['node', '-e', `
            ...
            Promise.resolve(fnModule.handler(event))
              .then(result => {
...
export default new DockerExecutionEngine();